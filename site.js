        ...
        document.querySelectorAll('.fade-in, .fade-in-left, .fade-in-right, .scale-in, .skill-progress').forEach(el => {
            observer.observe(el);
        });
        ...
        async function storeVisitorInfo(visitorInfo) {
            try {
                // Add to 'visitors' collection
                const docRef = await db.collection('visitors').add(visitorInfo);
            } catch (error) {
            }
        }
        ...
        async function getVisitorInfo() {
            try {
                ...
                consoleerror('Error getting visitor info:', error);
            }
        }
        ...
        function trackVisitorActions() {
            ...
            document.addEventListener('mousemove', (e) => {
                ...
                if (currentTime - lastRecordTime > 100) { // Record every 100ms
                    mouseMovements.push({
                        x: e.clientX,
                        y: e.clientY,
                        time: currentTime
            ...
            formInputs.forEach(input => {
                input.addEventListener('focus', () => {
                    console.log(`📝 Focused on: ${input.name || input.id || 'unnamed field'}`);
                });
            });
        }
